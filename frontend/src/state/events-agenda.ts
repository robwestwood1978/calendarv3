...
export function listExpanded(from: DateTime, to: DateTime, query: string): EventRecord[] {
  ...
  if (!featureAuthEnabled() || !myAgendaOn()) {
    // ✅ dedupe by (id,start) even when My Agenda is OFF, to avoid duplicates from overlays
    const out: EventRecord[] = []
    const seen = new Set<string>()
    for (const e of [...locals, ...externals]) {
  ...
  // ✅ union with (id,start) key to keep all occurrences across the month
  const out: EventRecord[] = []
  const seen = new Set<string>()
