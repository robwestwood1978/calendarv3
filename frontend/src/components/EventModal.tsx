...
import { calendarMetaFor, isExternal } from '../lib/external'
...
export default function EventModal({ open, initial, onClose, onSave }: Props) {
  ...
  const shadow = !!((initial as any)?.source === 'shadow')
  ...
  const nthInMonth = (dt: DateTime) => Math.ceil(dt.day / 7)
  const weekdayCode = (dt: DateTime) => WEEKDAY_CODES[dt.weekday - 1]
  const monthNum = (dt: DateTime) => dt.month
  ...
  const onSubmit = () => {
    ...
    // If this is a recurring event and the start changed, carry the original
    // occurrence start so the reducer can exdate the right instance.
    if (initial?.rrule && initial.start !== start) {
      (payload as any)._prevStart = initial.start
    }

    if (initial && isExternal(initial)) {
      // External edits go through shadow/tombstone flow; _prevStart hides the old instance.
      if (initial.start !== start) { (payload as any)._prevStart = initial.start }
  ...
  const onDelete = () => {
    ...
    if (isExternal(initial) || (initial as any)?.source === 'shadow') {
  ...
  return (
    <Portal>
      <div
        ...
        style={{ position: 'fixed', inset: 0, zIndex: 2147483000, background: 'rgba(0,0,0,0.15)' }}
      >
        <div className="modal modern" style={{ position: 'absolute', inset: '10% auto auto 50%', transform: 'translateX(-50%)', zIndex: 2147483001 }}>
            ...
            <section className="card" style={{ padding: '.6rem' }}>
              ...
              <div className="row" style={{ marginTop: '.35rem' }}>
                <select value={responsibleAdult} onChange={e => setResponsibleAdult(e.target.value)}>
                  ...
                  {members.filter(m => m.role === 'parent' || m.role === 'adult').map(m => m.name).map(n => <option key={n} value={n}>{n}</option>)}
                </select>
                  ...
                  <div>
                    ...
                    <div className="days">
                      {WEEKDAY_LABELS.map((d, idx) => {
                        ...
                        return <button key={code} className={`day ${on ? 'on' : ''}`} onClick={() => setByDays(prev => prev.includes(code) ? prev.filter(x => x !== code) : [...prev, code])}>{d}</button>
                      })}
                    </div>
                  </div>
                )}
