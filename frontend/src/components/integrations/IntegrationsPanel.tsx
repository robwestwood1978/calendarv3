...
function validateUrl(u: string): { ok: boolean, hint?: string } {
  ...
  let parsed: URL
  try { parsed = new URL(norm) } catch { return { ok: false, hint: 'Invalid URL. Use webcal:// or https:// to a public ICS.' } }
  const host = parsed.hostname.toLowerCase()
  const allow = [
    'icloud.com','apple.com',
    'google.com','calendar.google.com','googleusercontent.com',
    'outlook.com','outlook.office365.com','office365.com','office.com','live.com',
    'yahoo.com','calendar.yahoo.com',
    'teamup.com','ics.teamup.com'
  ]
...
export default function IntegrationsPanel(){
  ...
  function onAdd(e: React.FormEvent){
    ...
    setCals(listCalendars()); setUrl(''); setName(provider === 'apple' ? 'Apple Family' : 'External ICS'); setProvider('apple'); setError(null); setHint(null)
  }
  ...
  async function onRefresh(cal: ExternalCalendar){
    try {
      ...
      // Friendlier iCloud messaging
      const friendly = /403|401/i.test(msg) ? 'This iCloud calendar looks private. In Apple Calendar: Info → tick “Public Calendar”, then paste the public link.' :
                       /host.+allow/i.test(msg) ? 'This calendar host is not supported by the proxy allow-list.' :
                       /timeout|502/i.test(msg) ? 'Upstream timed out. Try again in a moment.' : null
        ...
        <>
          <form onSubmit={onAdd} style={{ display:'grid', gap:8, marginTop:12 }}>
            ...
            <div style={row}>
              ...
              <input
                value={url}
                onChange={e=>{ setUrl(e.currentTarget.value); onPreflight(e.currentTarget.value) }}
